...
- `account add` never prompts when non-interactive — it prints a one-line stderr tip pointing at
  `account soap-auth <alias>`; on a TTY it offers to chain SOAP setup immediately (answer `N` to
  skip). The add JSON gains `"soapTokenStored"` only when the chained setup actually ran.
